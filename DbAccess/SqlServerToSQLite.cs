...
namespace DbAccess
{
    ...
    {
        private static readonly string newGuidSQLite = "(lower(hex(randomblob(4))) || '-' || lower(hex(randomblob(2))) || '-4' || substr(lower(hex(randomblob(2))),2) || '-' || substr('89ab',abs(random()) % 4 + 1, 1) || substr(lower(hex(randomblob(2))),2) || '-' || lower(hex(randomblob(6))))";

        private static string ConvertGuid(Guid g)
        {
            return g.ToString();
        }
        ...
        private static void ValidateDataType(string dataType)
        {
            if (dataType == "int" || dataType == "smallint" ||
                ...
                dataType == "date" ||
                dataType == "time" ||
                dataType == "datetime" || dataType == "datetime2" || dataType == "datetimeoffset")
                return;
